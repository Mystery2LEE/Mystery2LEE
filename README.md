...
금융 규정과 공공 데이터를 검증 가능한 AI 서비스로 만드는 일을 합니다.<br/>
룰 엔진, RAG, 데이터 파이프라인을 설계하고 지표로 개선해 왔습니다.
...
### DarkAudit | 금융상품 다크패턴 점검 도구
`2026 금융 AI Challenge` `2인 팀` `Data Engineer`
...
### 보살핌 | Graph RAG 복약 안전 분석
`캡스톤 디자인` `4인 팀 팀장` `2026.03 ~ 2026.06`

여러 병원에서 약을 처방받는 고령자·1인 가구를 위해, 함께 먹으면 위험한 약 조합을 찾아 알려주는 기능을 만들었습니다.

- 식약처 DUR API로 성분 1,473개, 병용금기 관계 1,313개를 Neo4j 지식그래프로 구축
- 2홉 Cypher 쿼리로 직접 연결되지 않은 간접 위험 경로까지 탐지
- CODEF API로 처방전 데이터 연동
- 위험 판단은 그래프가, 설명은 LLM(GPT-4o-mini)이 맡도록 역할 분리
...
![footer](https://capsule-render.vercel.app/api?type=waving&color=0:1D4E89,100:0B2545&height=120&section=footer)